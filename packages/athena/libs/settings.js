...
module.exports = function (logger, t, noInterval, noAutoRun) {
	...
	*/
	...
	settings.update_settings = async (options, cb) => {
		...
		try {
			const results = await t.async.parallel([
			...
			//-----------------------------
			console.log('JSON Validation Docs, results[1] => ', results[1]);
			if (results[1] && results[1].rows) {
				for (let i in results[1].rows) {										// loop over all validation docs
					...
					// v2 & v3 validation - store the highest versioned swagger doc for each major version we find
						...
						console.log('settings.OPEN_API_DOCS[db_doc_major].info', doc.info.version, db_doc_major, settings.OPEN_API_DOCS[db_doc_major]);
						if (settings.OPEN_API_DOCS[db_doc_major] && settings.OPEN_API_DOCS[db_doc_major].info) {
							console.log('settings.OPEN_API_DOCS[db_doc_major].info', settings.OPEN_API_DOCS[db_doc_major].info);
				...
				console.log('settings.OPEN_API_DOCS keys', Object.keys(settings.OPEN_API_DOCS));
