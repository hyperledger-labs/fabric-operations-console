...
func main() {
	...
	options := []Option{
		WithCorsForRegisteredEndpointsOnly(false),
		WithOriginFunc(makeHttpOriginFunc(allowedOrigins)),
	}
	...
	if *useWebsockets {
		...
		options = append(
			options,
			WithWebsockets(true),
			WithWebsocketOriginFunc(makeWebsocketOriginFunc(allowedOrigins)),
		)
		...
		options = append(
			options,
			WithWebsocketPingInterval(*websocketPingInterval),
		)
		...
		options = append(
			options,
			WithWebsocketCompressionMode(compressionMode),
		)
	}
...
func buildServer(wrappedGrpc *WrappedGrpcServer, handler http.Handler) *http.Server {
	return &http.Server{
...
func runHealthChecker(ctx context.Context, backendConn *grpc.ClientConn, service string) *healthChecker {
	...
	go func() {
		err := ClientHealthCheck(ctx, backendConn, service, h.setServing)
		if err != nil {
