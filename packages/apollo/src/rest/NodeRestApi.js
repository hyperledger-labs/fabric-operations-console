...
class NodeRestApi {
	...
	//static REQ_LIMIT = 2;				// max number of parallel http requests
	...
	*/
	...
	static async getChannels(identities, osn) {
		const identity4tls = await NodeRestApi.findMatchingIdentity({
	...
	static async mapChannels(identities, osns) {
		...
		if (Array.isArray(osns)) {
			...
			for (let i in osns) {
				...
				const resp = await NodeRestApi.getChannels(identities, osns[i]);
				...
				if (resp && Array.isArray(resp.channels)) {
					...
					for (let z in resp.channels) {
						...
						if (!mapped_channel_names.includes(ch.name)) {
							...
							ret.channels.push(await NodeRestApi.map1Channel(identities, osns, ch.name));
						}
					}
				}
			}
			...
			if (last_ch_list_resp && last_ch_list_resp.systemChannel) {
				...
				ret.systemChannel = await NodeRestApi.map1Channel(identities, last_osn, sys_name);
			}
	...
	*/
	...
	static async map1Channel(identities, osns, channel) {
		if (Array.isArray(osns)) {
			...
			// iter on each osn in input
			/*for (let i in osns) {
				const ch_details = await NodeRestApi.get1Channel(identities, osns[i], channel);
			...
			let reqs = osns.map(async osn => {
				...
				const ch_resp = await NodeRestApi.get1Channel(identities, osn, channel);
